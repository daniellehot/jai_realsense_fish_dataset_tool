...
def process_img(_depth_path, _num):
    # Load depth map
    depth_map = o3d.io.read_image(_depth_path)

    # Convert to pointcloud
    pcd = depthmap2pointcloud(depth_map)
    ...
    # Save mask
    cv2.imwrite("mask{0}.png".format(_num), mask)

    # Save debug image
    rgb_img = cv2.imread(_depth_path.replace("/depth/","/rgb/"))
    plt.imshow(rgb_img)
    plt.imshow(mask, alpha=0.3, cmap='gist_ncar')
    plt.savefig("mask-overlaid{0}.png".format(_num))


if __name__ == "__main__":
    test_path = "../../data_depth_analysis/data/rs/depth/00032.png"
